...
std::vector<PowerPlant> * IOFile::loadPowerplants(std::vector<Player *> player_vector) {
	...
	if (powerplantInput.is_open())
	{
		...
		while (getline(powerplantInput, line) && (line != "") && !line.empty()) {
			...
			if (player_name != "") {
				for (unsigned int i = 0; i < player_vector.size(); i++) {
					if (player_name == player_vector[i]->getName()) {
						player_vector[i]->addPowerPlant(powerplant_Obj);
						player_vector[i]->setnumOfPowerPlants(player_vector[i]->getnumOfPowerPlants());
					}
				}
			}
